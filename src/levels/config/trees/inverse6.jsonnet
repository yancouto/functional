  ...
  num.test_list_num('f: f %s' % [lib.bst(list)], [x for x in std.range(1, 6) if std.find(x, list) == []]);
{
  ...
  solutions: [
    local list_6 = '(PUSH 1 (PUSH 2 (PUSH 3 (PUSH 4 (PUSH 5 (PUSH 6 FALSE))))))',
          not_contains = '(Y (f: b:x: b (l:v:r:t: (EQ v x) FALSE ((ZERO (SUB v x)) (f r x) (f l x))) TRUE))';
    'b: FILTER %s (x: %s b x)' % [list_6, not_contains],
